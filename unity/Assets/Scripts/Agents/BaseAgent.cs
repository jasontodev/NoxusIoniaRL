    ...
    public class BaseAgent : Agent
    {
        ...
        private void HandleInteract()
        {
            ...
            if (manaCarried > 0)
            {
                ...
                {
                    if (healZone.DepositMana(this, manaCarried))
                    {
                        var depositReward = rewardDeposit * manaCarried;
                        AddReward(depositReward);
                        manaCarried = 0;
                        eventLogger?.LogEvent("deposit", agentId, teamType, new Dictionary<string, object> { { "amount", manaCarried } });
                    }
                }
            }
